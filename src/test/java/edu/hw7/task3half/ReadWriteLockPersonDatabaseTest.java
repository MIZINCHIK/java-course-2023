...
public class ReadWriteLockPersonDatabaseTest {
    ...
    void addAndFind_singleThreadAddingAndFinding_personsCanBeFound() {
        var database = new ReadWriteLockPersonDatabase();
    ...
    private void doSupply(ReadWriteLockPersonDatabase database) {
    ...
    private void assertConsume(ReadWriteLockPersonDatabase database) {
        for (int i = 0; i < 1000; i++) {
            AssertionsForClassTypes.assertThat(database.findByPhone("111").size()
